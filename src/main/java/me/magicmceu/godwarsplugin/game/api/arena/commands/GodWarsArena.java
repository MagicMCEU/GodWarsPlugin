...
public class GodWarsArena implements CommandExecutor {
    ...
    private final List<String> args0 = Arrays.asList("add", "finish");
    private final List<String> arenaType = Arrays.asList("solo", "doubles", "triples", "4v4v4v4", "4v4");
    private boolean shouldBreakCheckingMaps;

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        String cfgLoc = "maps.";
        Boolean canMake = Boolean.TRUE;
                    ...
                    if (args[0].equalsIgnoreCase("add")) {
                        ...
                        if (config.contains("maps")) {
                            config.getConfigurationSection("maps").getKeys(false).forEach(key -> {
                                if (config.getString("maps." + key + ".name").equals(args[1]) && !shouldBreakCheckingMaps) {
                        ...
                        if (arenaType.contains(args[2].toLowerCase())) {
                            ...
                            // if(config.get(cfgLoc + ".test") == null) p.sendMessage("Działa");
                            break;
                        }
